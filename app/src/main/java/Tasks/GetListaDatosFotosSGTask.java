...
public class GetListaDatosFotosSGTask extends AsyncTask<String,String,ArrayList<DocsSugerencia>> {
    ArrayList<DocsSugerencia> result;
    @Override
    protected ArrayList<DocsSugerencia> doInBackground(String... strings) {
        ArrayList<DocsSugerencia> LstData = new ArrayList<DocsSugerencia>();
        final String NAMESPACE = Constans.NameSpaceWS;
        final String URL = Constans.UrlServer;
        final String METHOD_NAME = "GetListFotosSG";
        ...
        try {
            ...
            Log.i("result get lista datos fotos SG ", resSoap.toString());
            for (int i = 0; i < num_projects; i++) {
                SoapObject ic = (SoapObject) resSoap.getProperty(i);

                DocsSugerencia oEnt = new DocsSugerencia();
                oEnt.setC_compania(ic.getPrimitivePropertyAsString("c_compania"));
                oEnt.setN_sugerencia( Integer.valueOf( ic.getPrimitivePropertyAsString("n_sugerencia")));
            ...
            Log.i("AsynckTask get lista datos fotos SG ---", e.getMessage());
        }
        return result;
    }
}
